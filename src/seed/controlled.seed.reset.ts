...
async function runReset() {
  ...
  try {
    ...
    await prisma.foodIngredient.deleteMany({
      where: { ingredientId: { in: seedIngredientIds } },
    });
    await prisma.ingredientAllergen.deleteMany({
      where: { ingredientId: { in: seedIngredientIds } },
    });
    await prisma.ingredient.deleteMany({
      where: { id: { in: seedIngredientIds } },
    });
    ...
    await prisma.foodNutritionValue.deleteMany({
      where: { foodNutritionProfileId: { in: profileIds } },
    });
    await prisma.foodNutritionProfile.deleteMany({
