...
async function checkSeed() {
  ...
  console.log(
    `Seed users total: ${seedUsers}  (expected ≥ 72 — 10 SC + 50 cluster + 10 affinity + 10 pop + 2 reset pools)`,
  );
  ...
  console.log(
    `SC02 allergies: [${sc02?.allergies.map((a) => a.allergen.name).join(', ')}]  (expected: Shellfish, Fish)`,
  );

  // 5. SC10 goal type (expected: GOAL_STRICT - keyMap từ AllCode)
  const sc10 = await prisma.user.findUnique({
    where: { email: 'sc10@seed.test' },
    include: {
      nutritionGoals: { select: { goalType: true, targetCalories: true } },
    },
  });
  console.log(
    `SC10 goalType: ${sc10?.nutritionGoals[0]?.goalType}  (expected: GOAL_STRICT), calories=${sc10?.nutritionGoals[0]?.targetCalories?.toFixed(0)}`,
  );

  // 6. SC01 remaining today
  const sc01 = await prisma.user.findUnique({
    where: { email: 'sc01@seed.test' },
  });
  if (sc01) {
    ...
    const consumed =
      log?.meals
        .flatMap((m) => m.mealItems)
        .reduce((s, i) => s + i.calories, 0) ?? 0;
    const goal = await prisma.nutritionGoal.findFirst({
      where: { userId: sc01.id },
    });
    const remaining = goal ? goal.targetCalories - consumed : 0;
    console.log(
      `SC01 today: consumed=${consumed.toFixed(0)} kcal, target=${goal?.targetCalories.toFixed(0)}, remaining=${remaining.toFixed(0)}  (expected remaining ~500)`,
    );
  }
  ...
  newFoods.forEach((f) => {
    ...
    console.log(
      `  ${f.foodName} — ${daysOld.toFixed(1)} days ago  (expected ≤ 3)`,
    );
  });
  ...
  const shellfishFish = await prisma.ingredient.findMany({
    where: {
      ingredientName: {
        in: ['Shellfish ingredient (seed)', 'Fish ingredient (seed)'],
      },
    },
    include: {
      foodIngredients: { include: { food: { select: { foodName: true } } } },
      ingredientAllergens: {
  ...
  for (const ing of shellfishFish) {
    ...
    ing.foodIngredients.forEach((fi) =>
      console.log(`  [${allergenName}] ${fi.food.foodName}`),
    );
  }
